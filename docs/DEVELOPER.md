...
├── editor.js       # CodeMirror 에디터 설정 및 상호작용
├── renderer.js     # 마크다운 렌더링, 내부 링크 처리, 목차(TOC) 생성
├── sidebar.js      # 사이드바 관리
...
모든 애플리케이션 상태는 중앙 상태 관리 모듈인 `store.js`를 통해 관리됩니다. 중앙 상태 관리 객체는 명확성을 위해 기존 `state`에서 `store`로 이름이 변경되었습니다.
...
### 사이드바 (Sidebars)
  ...
  * **우측 (Sidebar Right)**:
      * **목차 (TOC)**: `renderer.js` 모듈이 렌더링된 HTML의 `h1`\~`h6` 태그를 파싱하여 동적으로 생성합니다. 나무위키 스타일의 넘버링 로직을 포함합니다.
...
모든 마크다운 렌더링 관련 로직은 `renderer.js` 모듈에 캡슐화되어 있습니다. 이 모듈은 `marked.parse(text)`를 사용하여 HTML로 변환하는 작업을 담당합니다.
**내부 링크 처리** 또한 렌더링 후 `renderer.js` 내의 `attachInternalLinkHandlers()`를 통해 `<a>` 태그를 가로채서, 외부 링크가 아닌 경우 SPA 내부 라우팅으로 처리합니다.
